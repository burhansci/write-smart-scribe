...
interface CohesionAnalysisProps {
  cohesionAnalysis: string;
}

const CohesionAnalysis = ({ cohesionAnalysis }: CohesionAnalysisProps) => {
  // Parse before/after examples from the cohesion analysis text
  ...
  const examples = parseBeforeAfterExamples(cohesionAnalysis);
  const generalAnalysis = cohesionAnalysis.split('BEFORE→AFTER')[0].trim();
      ...
      <CardContent className="space-y-6">
        {generalAnalysis && (
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <p className="text-purple-800 leading-relaxed">{generalAnalysis}</p>
          </div>
        )}
        ...
        {examples.length === 0 && !generalAnalysis && (
          <p className="text-gray-500 italic">No specific cohesion improvements identified in this analysis.</p>
        )}
      </CardContent>
    </Card>
  );
...
export default CohesionAnalysis;
